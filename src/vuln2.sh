...
# Subdomain Finder
	printf "\e[1;91m \n[+] Finding Subdomains\e[0m\n"
	mkdir -p ./subdomains
	subfinder -d $domain -silent | tee -a ./subdomains/subfinder.txt > /dev/null 2>&1
...
for line in $(cat ./subdomains/subfinder.txt );
do
